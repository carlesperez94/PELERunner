...
def add_csv_column_to_csv(csv_original, csv_to_add_path, sep_o=",", sep_a=",", col_names_to_add=("total_hbonds",
    ...
    dt_original = pd.read_csv(csv_original, sep=sep_o)
    dt_to_add = pd.read_csv(csv_to_add_path, sep=sep_a)
...
folder_list = glob.glob("/home/carlespl/project/Almirall/validation_compounds/simulation_results_with_exp_data/"
                        "*/obc_adaptive*/hbond_summary_completed.csv")
csv_list = glob.glob("/home/carlespl/project/Almirall/validation_compounds/adaptive_results_summary/*")
for folder in folder_list:
    folder_id = folder.split("/")[-3].split("_")[0]
    for csv in csv_list:
